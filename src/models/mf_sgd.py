...
class MatrixFactorizationSGD(BaseMatrixFactorization):
    ...
    def _compute_validation_metric(self, val_data: np.ndarray, metric_name: str) -> float:
        """
        计算验证集上的指定指标
        ...
        if metric_name == 'val_loss':
            return self.compute_loss(val_predictions, val_targets)
        elif metric_name == 'val_rmse':
            return np.sqrt(np.mean((val_predictions - val_targets) ** 2))
        elif metric_name == 'val_mae':
            return np.mean(np.abs(val_predictions - val_targets))
        elif metric_name == 'val_mse':
            return np.mean((val_predictions - val_targets) ** 2)
        else:
            # 默认使用损失函数
            logger.warning(f"未知的验证指标 '{metric_name}'，使用默认的 'val_loss'")
            return self.compute_loss(val_predictions, val_targets)

    def fit(self, train_data: np.ndarray, val_data: Optional[np.ndarray] = None,
            n_epochs: int = 100, batch_size: Optional[int] = None,
            verbose: int = 1, early_stopping_patience: Optional[int] = 10,
            early_stopping_metric: str = 'val_loss', minimize_metric: bool = True,
        ...
        Args:
            ...
            verbose: 日志详细程度
            early_stopping_patience: 早停耐心值，设置为None禁用早停
            early_stopping_metric: 早停使用的指标，可选: 'val_loss', 'val_rmse', 'val_mae', 'val_mse'
            minimize_metric: 是否最小化指标（True表示指标越小越好）
            shuffle: 是否打乱数据
            callbacks: 回调函数列表
        """
        # 初始化参数（如果尚未初始化）
        ...
        # 早停相关变量
        use_early_stopping = (early_stopping_patience is not None) and (val_data is not None)
        if use_early_stopping:
            best_metric_value = np.inf if minimize_metric else -np.inf
            patience_counter = 0

            # 验证早停指标是否有效
            supported_metrics = ['val_loss', 'val_rmse', 'val_mae', 'val_mse']
            if early_stopping_metric not in supported_metrics:
                logger.warning(f"不支持的早停指标 '{early_stopping_metric}'，使用默认的 'val_loss'")
                early_stopping_metric = 'val_loss'

            if verbose > 0:
                logger.info(f"启用早停: 使用指标 '{early_stopping_metric}'，耐心值={early_stopping_patience}")
        else:
            if verbose > 0:
                if early_stopping_patience is None:
                    logger.info("早停已禁用")
                elif val_data is None:
                    logger.info("未提供验证数据，早停已禁用")

        # 训练历史记录扩展
        self.train_history['metric_used'] = early_stopping_metric if use_early_stopping else None
        ...
        for epoch in range(n_epochs):
            ...
            if val_data is not None:
                ...
                # 如果启用早停，计算指定的验证指标
                if use_early_stopping:
                    metric_value = self._compute_validation_metric(val_data, early_stopping_metric)
                    ...
                    if improved:
                        ...
                        if verbose > 1:
                            logger.info(f"  找到更好的模型: {early_stopping_metric}={metric_value:.4f}")
                    else:
                        patience_counter += 1
                    ...
                    if patience_counter >= early_stopping_patience:
                        if verbose > 0:
                            logger.info(f"早停触发在epoch {epoch+1}: {early_stopping_metric}在{early_stopping_patience}轮内未改善")
                            logger.info(f"最佳{early_stopping_metric}: {best_metric_value:.4f}")
                        # 恢复最佳参数
            ...
            if verbose > 0:
                ...
                if val_data is not None:
                    ...
                    if use_early_stopping and early_stopping_metric != 'val_loss':
                        log_msg += f" - {early_stopping_metric}: {metric_value:.4f}"
                logger.info(log_msg)
        ...
        # 训练结束
        if verbose > 0:
            if use_early_stopping and hasattr(self, '_best_params'):
                logger.info(f"训练完成，使用最佳模型参数（基于{early_stopping_metric}）")
            else:
                logger.info(f"训练完成，共{epoch+1}轮")
