...
def main():
    ...
    print("1. 注册豆瓣电影数据集...")
    # 确保注册成功
    DatasetLoader.register_dataset('doubanmovies', MovieLens100K)
    # 验证注册是否成功
    ...
    # 步骤4: 加载和预处理数据
    print("\n3. 加载豆瓣电影数据...")
    data_path = 'E:\工作资料\科研\论文\写作\使用混合分段损失函数增强矩阵分解用于推荐\矩阵分解损失函数code\dataset\\20201202M100K_test.txt'  # 你的数据文件路径
...
def quick_usage():
    ...
    data_manager.load_dataset('doubanmovies', 'E:\工作资料\科研\论文\写作\使用混合分段损失函数增强矩阵分解用于推荐\矩阵分解损失函数code\dataset\\20201202M100K_test.txt').preprocess()
