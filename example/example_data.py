...
def main():
    ...
    # 数据文件路径（请根据实际情况修改）
    data_path = 'E:\工作资料\科研\论文\写作\使用混合分段损失函数增强矩阵分解用于推荐\矩阵分解损失函数code\dataset\20201202M100K_test.txt'
    
    try:
        # 加载和预处理数据（链式调用）
        data_manager.load_dataset('20201202M100K_test', data_path).preprocess()
        ...
        # 获取训练集的批处理迭代器
        train_iterator = data_manager.get_batch_iterator('train', batch_size=64)
    ...
    try:
        ...
        # ID重新索引
        reindexed_data, user_map, user_inv_map, item_map, item_inv_map = preprocessor.reindex_ids(raw_data)
