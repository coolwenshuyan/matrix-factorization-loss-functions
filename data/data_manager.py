...
class DataManager:
    ...
    def load_dataset_from_path(self, path: str,
                            ...
                            format_type: str = "auto") -> 'DataManager':
        """
        直接从路径加载数据集
        ...
        Args:
            path: 数据文件路径
            name: 数据集名称（可选）
            rating_scale: 评分范围（可选）
            format_type: 数据格式类型（可选）

        Returns:
            self，支持链式调用
        """
        logger.info(f"从路径加载数据集: {path}")

        # 使用增强的加载器
        self.dataset = DatasetLoader.load_dataset_from_path(
            path, name, rating_scale, format_type
        )
    ...
    def quick_load_and_preprocess(self, path: str) -> 'DataManager':
        """
        快速加载并预处理数据集
        ...
        Returns:
            self，支持链式调用
        """
        return self.load_dataset_from_path(path).preprocess()

    # 添加一个静态方法用于最简单的使用
    @staticmethod
    def create_from_path(path: str, config: Optional[Dict] = None) -> 'DataManager':
        """
        静态工厂方法，从路径创建数据管理器
        ...
        return manager.load_dataset_from_path(path)
...
if __name__ == "__main__":
    ...
    # data_manager.load_dataset_from_path('E:\工作资料\科研\论文\写作\使用混合分段损失函数增强矩阵分解用于推荐\矩阵分解损失函数codeCPU\dataset\small_20201202M100K_data_all_random_1percent.txt')
    # data_manager.preprocess()
